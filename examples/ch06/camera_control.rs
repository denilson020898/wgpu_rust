...
impl State {
    async fn new(window: &Window) -> Self {
        ...
        let camera = camera::Camera::new((2.0, 2.0, 2.0), cgmath::Deg(-80.0), cgmath::Deg(-30.0));
    ...
    pub fn input(&mut self, event: &DeviceEvent) -> bool {
        match event {
            ...
            DeviceEvent::MouseMotion { delta } => {
                println!(
                    "mouse pressed: {}, delta is : {:?}",
                    self.mouse_pressed, delta
                );
...
fn main() {
    ...
    window.set_title(&*format!("{}", "ch06_camera-control"));
